...
import { logout as authLogout, validateToken } from '../utils/authEvents'
import type { User, Survey, ApiResponse, ApiError } from '../types'
...
async function fetchUserSurveys() {
  ...
  surveysError.value = ''
  try {
    const res = await apiClient.get<ApiResponse<Survey[]>>(`/api/surveys/user?page=${page.value}&size=${size.value}`)
    surveys.value = res.data.data || []
    totalPages.value = res.data.pagination?.totalPages || 1
  } catch (e) {
    const apiError = e as ApiError
    surveysError.value = apiError?.message || 'Failed to fetch surveys.'
  } finally {
...
onMounted(async () => {
  ...
  try {
    const res = await apiClient.get<ApiResponse<User>>(`/api/users/username/${username}`)
    ...
    const apiError = e as ApiError
    error.value = apiError?.message || 'Failed to fetch profile.'
    redirecting.value = true
